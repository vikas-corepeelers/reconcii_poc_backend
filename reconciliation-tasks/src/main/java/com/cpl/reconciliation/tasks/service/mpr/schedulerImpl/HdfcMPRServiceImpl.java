...
//import com.cpl.reconciliation.tasks.service.mpr.processorImpl.HdfcMPRProcessorImpl;
...
//@Slf4j
//@Service
//public class HdfcMPRServiceImpl extends AbstractService implements DataService {
//    private final static String fileSeparator = System.getProperty("file.separator");
//    private final DataEntryLogDao dataEntryLogDao;
//    private final MailService emailService;
//    private final MPRDao mprDao;
//    private final HdfcMPRProcessorImpl hdfcProcessor;
//    @Value("${sftp.root}")
//    private String sftpRoot;
//
//    @Override
//    public DataSource getDataSource() {
//        return DataSource.HDFC_MPR;
//    }
//
//    @Override
...
//                String path = sftpRoot + fileSeparator + "hdfc" + fileSeparator + "mpr" + fileSeparator + businessDate;
//                File folder = new File(path);
//                if (isDirectoryWithContent(folder)) {
//                    File[] files = folder.listFiles(extentionFilter(".eml"));
//                    log.info("HDFC MPR folder {} file count: {}", path, files.length);
//                    for (File file : files) {
//                        MimeMessage msg = EmailConverter.emlToMimeMessage(file);
//                        try {
//                            hdfcProcessor.processMessages(new Message[]{msg});
//                        } catch (Exception e) {
//                            log.error("Error while parsing HDFC MPR mail: " + file.getName() + " : " + e.getMessage(), e);
//                        }
//                    }
//                    logInDB(stringToLocalDate(businessDate, Formatter.YYYYMMDD_DASH), files.length);
//                } else {
//                    log.error("HDFC MPR folder does not exists: {}", path);
//                }
//            } else {
//                log.info("file already parsed for today");
//            }
//        } catch (Exception e) {
//            log.error("Exception occurred while reading HDFC MPR email: ", e);
//        }
//    }
//
//    @Override
...
//                hdfcProcessor.processZipFile(inputStream);
//            }
//            mprDao.getMprBankDifference(Bank.HDFC.name(), PaymentType.UPI.name(), businessDate.format(com.cpl.core.api.constant.Formatter.YYYYMMDD_DASH), endDate.format(Formatter.YYYYMMDD_DASH));
//            mprDao.getMprBankDifference(Bank.HDFC.name(), PaymentType.CARD.name(), businessDate.format(com.cpl.core.api.constant.Formatter.YYYYMMDD_DASH), endDate.format(Formatter.YYYYMMDD_DASH));
