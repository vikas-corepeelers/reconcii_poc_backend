...
//import com.cpl.reconciliation.domain.models.PhonePayMPR;
...
//@Slf4j
//@Service
//public class PhonePayMPRProcessorImpl implements MessageProcessor {
//
...
//    private static final SimpleDateFormat dateTimeFormat = new SimpleDateFormat("MM/dd/yyyy");
//
//
...
//        String fileType = "zip";
//        String mprSource = System.getProperty("java.io.tmpdir") + "mpr" + File.separator + "yes";
...
//        Map<String, String> customisedAndActualFieldsMap = customisedFieldsMappingDao.getActualAndCustomisedFieldsMapByDataSource(DataSource.PHONEPE_MPR);
...
//                .build();
//        try {
//            List<PhonePayMPR> phonePayMPRS = Poiji.fromExcel(file, PoijiExcelType.XLSX, PhonePayMPR.class, options);
//
//            List<MPREntity> mprEntities = phonePayMPRS.stream().map(phonePayMPR -> {
//                MPREntity mprEntity = new MPREntity();
//                mprEntity.setBank(Bank.PHONEPE);
//                mprEntity.setTid(phonePayMPR.getTerminalId());
//                mprEntity.setTransactionDate(parseDate(phonePayMPR.getTransactionDate()));
//                mprEntity.setSettledDate(parseDate(phonePayMPR.getSettlementDate()));
//                mprEntity.setMprAmount(phonePayMPR.getAmount());
//                mprEntity.setSettledAmount(phonePayMPR.getCreditAmt());
//                mprEntity.setTransactionId(phonePayMPR.getPhonePeReferenceId());
//                mprEntity.setCommission(phonePayMPR.getFee());
//                mprEntity.setCgst(phonePayMPR.getCGST());
//                mprEntity.setIgst(phonePayMPR.getIGST());
//                mprEntity.setSgst(phonePayMPR.getSGST());
//                mprEntity.setPaymentType(PaymentType.UPI);
//                mprEntity.setMerchantRefId(phonePayMPR.getMerchantReferenceId());
//                mprEntity.setTransactionUtr(phonePayMPR.getTransactionUTR());
//                mprEntity.setRefund(true ? phonePayMPR.getPaymentType().toUpperCase().contains("REFUND") : false);
//                Optional<StoreTIDMapping> storeTIDMapping = storeTIDMappingDao.findByTid(mprEntity.getTid());
//                mprEntity.setStoreId(storeTIDMapping.isPresent() ? storeTIDMapping.get().getStoreCode() : null);
//                mprEntity.setBankCharges(mprEntity.getCommission() + mprEntity.getIgst() + mprEntity.getCgst() + mprEntity.getSgst());
//                String id = mprEntity.getTransactionId() + "_" + mprEntity.getMprAmount();
//                mprEntity.setId(id);
//
//                return mprEntity;
//            }).collect(Collectors.toList());
//            List<List<MPREntity>> chunks = utility.chunkList(mprEntities, 500);
//            for (List<MPREntity> chunk : chunks) {
//                mprDao.saveAll(chunk);
//                log.info("Records for PhonePay Mpr Imported : {}", chunk.size());
//            }
//
//            String response = "PhonePay Data imported - " + chunks.size();
//            return response;
//        } catch (Exception e) {
