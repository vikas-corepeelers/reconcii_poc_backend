...
FROM
    ...
    FROM
        mcd.mpr
    WHERE
        bank = 'HDFC' AND payment_type = 'CARD'
    ...
    FROM
        mcd.bank_statements
    WHERE
        source_bank = 'HDFC' AND payment_type = 'CARD'
...
SELECT date(settled_date), SUM(settled_amount), count(*) from mcd.mpr WHERE bank="HDFC" AND payment_type="CARD" AND bs_matched=false group by date(settled_date);