...
FROM
    ...
    FROM
        mcd.mpr
    WHERE
        bank = 'AMEX' AND payment_type = 'CARD'
    ...
    FROM
        mcd.bank_statements
    WHERE
        source_bank = 'AMEX' AND payment_type = 'CARD'
...
SELECT date(settled_date), SUM(settled_amount), count(*) from mcd.mpr WHERE bank="AMEX" AND payment_type="CARD" AND bs_matched=false group by date(settled_date);
UPDATE mcd.mpr SET bs_matched = TRUE WHERE date(settled_date)="2023-11-12" AND bank="AMEX" AND payment_type="CARD";