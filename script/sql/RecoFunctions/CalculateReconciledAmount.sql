DROP FUNCTION IF EXISTS `CALCULATE_RECONCILED_AMOUNT`;
DELIMITER //
CREATE FUNCTION CALCULATE_RECONCILED_AMOUNT(
    billSubTotal DOUBLE,
    finalAmount DOUBLE,
    freebies DOUBLE,
...
BEGIN
   ...
   SET unreconciledAmount = IS_ORDER_RECONCILED(
    ...
    IF unreconciledAmount=0 THEN
        SET reconciledAmount = threePOSales;
    ELSE
        SET reconciledAmount = 0; 
    END IF;


    RETURN ROUND(reconciledAmount,0);
END //
